...
@section('container')
    <section class="px-5 md:pl-24 md:pr-20 my-10 md:ml-56 flex-grow mt-[100px]">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            ...
            <div class="bg-white p-6 rounded-lg shadow">
                <h2 class="text-xl font-bold text-gray-700">Active Districts</h2>
                <p class="text-3xl font-semibold text-green-500 mt-2">{{ $activeDistricts }}</p>
            </div>
            <div class="bg-white p-6 rounded-lg shadow">
                <h2 class="text-xl font-bold text-gray-700">Active Sub-Districts</h2>
                <p class="text-3xl font-semibold text-green-500 mt-2">{{ $activeSubDistricts }}</p>
            </div>
            <div class="bg-white p-6 rounded-lg shadow">
                <h2 class="text-xl font-bold text-gray-700">New Data This Month</h2>
                <p class="text-3xl font-semibold text-yellow-500 mt-2">{{ $newDataThisMonth }}</p>
            </div>
        </div>

        <div class="bg-white p-6 rounded-lg shadow">
            <h2 class="text-xl font-bold text-gray-700 mb-4">Recent Data Entries</h2>
            <ul class="divide-y divide-gray-200">
                @foreach ($recentContacts as $contact)
                    <li class="py-4 flex justify-between items-center">
                        <div>
                            <h3 class="text-lg font-semibold text-gray-700">{{ $contact->office_name }} -
